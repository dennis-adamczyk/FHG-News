$(document).ready(function() {
  ...
  for(var i = 0; i < imgAmount; i++) {
    ...
    $imgContainer.append('<div class="image" style=\'background-image: url("img/thumb/' + (i+1) + '.jpeg")\'></div>');
  }

  $('.content .main .image-container .image').click(function(event) {
    var lightboxID = $(this).css('background-image').replace('url("https://duispaper.de/additional-content/qr/3/GlaeserneSchule/img/thumb/', '');
    lightboxID = lightboxID.replace('url("https://www.duispaper.de/additional-content/qr/3/GlaeserneSchule/img/thumb/', '');
  ...
  if(~window.location.href.indexOf("?lightbox")) {
    ...
    $('.content .lightbox .header .download').click(function(event) {
      if(image >= imgAmount) {
        ...
        link.download = 'Gläserne Schule - v' + (image - imgAmount) + '.mp4';
        document.body.appendChild(link);
        link.click();
      } else {
        var link = document.createElement('a');
        link.href = 'img/' + image + '.jpeg';
        link.download = 'Gläserne Schule - ' + image + '.jpeg';
        document.body.appendChild(link);
        link.click();
      }
    });
