$(document).ready(function() {
  ...
  $('.content .main .image-container .image').click(function(event) {
    var lightboxID = $(this).css('background-image').replace('url("https://duispaper.de/additional-content/qr/3/Weihnachtskonzert/img/thumb/', '');
    lightboxID = lightboxID.replace('url("https://www.duispaper.de/additional-content/qr/3/Weihnachtskonzert/img/thumb/', '');
  ...
  if(~window.location.href.indexOf("?lightbox")) {
    ...
    $('.content .lightbox .header .download').click(function(event) {
      if(image >= imgAmount) {
        ...
        link.download = 'Gläserne Schule - ' + image + '.jpg';
        document.body.appendChild(link);
        link.click();
      }
    });
