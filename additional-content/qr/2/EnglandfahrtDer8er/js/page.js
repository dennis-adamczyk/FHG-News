$(document).ready(function() {
  ...
  $('.content .main .image-container .image').click(function(event) {
    var lightboxID = $(this).css('background-image').replace('url("https://duispaper.de/additional-content/qr/2/EnglandfahrtDer8er/img/thumb/', '');
    lightboxID = lightboxID.replace('url("https://www.duispaper.de/additional-content/qr/2/EnglandfahrtDer8er/img/thumb/', '');
  ...
  if(~window.location.href.indexOf("?lightbox")) {
    ...
    $('.content .lightbox .header .download').click(function(event) {
      if(image >= imgAmount) {
        ...
        link.download = 'Englandfahrt der 8er - v' + (image - imgAmount) + '.mp4';
        document.body.appendChild(link);
        link.click();
      } else {
        var link = document.createElement('a');
        link.href = 'img/' + image + '.jpeg';
        link.download = 'Englandfahrt der 8er - ' + image + '.jpeg';
        document.body.appendChild(link);
        link.click();
      }
    });
