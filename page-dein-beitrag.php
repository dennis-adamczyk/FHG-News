...
if ( ! empty( $_POST ) ) {
	...
	if ( isset( $_REQUEST['suggestion'] ) ) {
		...
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'your-contribution_suggestion_' . get_current_user_id() ) ) {
			$script = "showSingleLineSnackBar('Verifikation fehlgeschlagen');";
		} else {
			$errors = array();

			$suggestion = strip_tags( trim( $_POST["suggestion"] ) );

			if ( ! isset( $suggestion ) || strlen( $suggestion ) == 0 ) {
				$errors['suggestion'] = "Dies ist ein Pflichtfeld";
			} else if ( strlen( $suggestion ) < 10 ) {
				$errors['suggestion'] = "Mindestens 10 Zeichen eingeben";
			}

			$email_msg = "<br><p><b>Themenvorschlag:</b><br>" . nl2br( $suggestion ) . "</p>";

			if ( count( $errors ) === 0 ) {
				if ( ! wp_mail( get_option( 'admin_email' ), "Themenvorschlag",
					"<h1>Neuer Themenvorschlag für die Redaktion</h1>
                  <h2>Benutzerdaten:</h2>
                  " . ( is_user_logged_in() ? "
                  <p>
				  ...
				  <?php if ( isset( $_REQUEST['success'] ) ): ?>
                    <div class="success">
                      <img src="<?php echo get_theme_file_uri( '/img/undraw/order_confirmed.svg' ); ?>"
                           alt="Anfrage gesendet">
                      <h3>
						  <?php if ( isset( $_REQUEST["suggestion"] ) ): ?>
                            Dein Themenvorschlag wurde an uns übermittelt
						  <?php else: ?>
                      ...
                      <p>
						  <?php if ( isset( $_REQUEST["suggestion"] ) ): ?>
                            Danke für deinen Vorschlag! Wir haben uns deinen Vorschlag notiert und werden versuchen uns darüber zu informieren.
						  <?php else: ?>
                            Der Überprüfungsvorgang kann ein paar Tage dauern. Behalte solange dein E-Mail-Postfach im Auge.
						  <?php endif; ?>
                      </p>
                    </div>
				  <?php else: ?>
                    ...
                    <div class="tabs">
                      <a href="." class="tab<?php echo( isset( $_REQUEST["suggestion"] ) ? "" : " active" ); ?>">
                        Konto freischalten
                      </a>
                      <a href="?suggestion"
                         class="tab<?php echo( isset( $_REQUEST["suggestion"] ) ? " active" : "" ); ?>">
                        Thema vorschlagen
                      </a>
                    </div>

					  <?php if ( isset( $_REQUEST['suggestion'] ) ): ?>
						  <?php wp_nonce_field( 'your-contribution_suggestion_' . get_current_user_id() ); ?>
                      <div
                          class="suggestion input input--textarea <?php echo( ! empty( $errors['suggestion'] ) ? "isInvalid" : "" ); ?>">
                      <textarea name="suggestion" id="suggestion"
                                placeholder="Beschreibe dein Thema"
                                required><?php echo( isset( $_POST['suggestion'] ) ? $_POST['suggestion'] : "" ); ?></textarea>
                        <label for="suggestion" class="label">Vorschlag</label>
                        <label for="suggestion"
                               class="error"><?php echo( ! empty( $errors['suggestion'] ) ? $errors['suggestion'] : "" ); ?></label>
                      </div>
                      ...
                      <div class="type select">
                        ...
                        <label class="styledSelect">
                          <select id="type" name="type">
                            ...
                            <option <?php echo( $_POST["type"] === "principal" ? "selected" : "" ) ?>
                                value="principal">
                      ...
                      <div class="changeable">
                        <div class="student">
                          <div
                              class="name input <?php echo( ! empty( $errors['student-name'] ) ? "isInvalid" : "" ); ?>">
                          ...
                          <div
                              class="age input <?php echo( ! empty( $errors['student-age'] ) ? "isInvalid" : "" ); ?>">
                      ...
                      <textarea name="student-message" id="student-message"
                        ...
                        <div class="teacher">
                          <div
                              class="name input <?php echo( ! empty( $errors['teacher-name'] ) ? "isInvalid" : "" ); ?>">
                      ...
                      <textarea name="teacher-message" id="teacher-message"
                        ...
                        <div class="parent">
                          <div
                              class="name input <?php echo( ! empty( $errors['parent-name'] ) ? "isInvalid" : "" ); ?>">
                          ...
                          <div
                            ...
                            <input type="text" name="parent-name_child" id="parent-name_child"
                                   ...
                                   required">
                            <i class="material-icons">cancel</i>
                            <label for="parent-name_child" class="label">Vor- und Nachname des Kindes auf dem
                              FHG</label>
